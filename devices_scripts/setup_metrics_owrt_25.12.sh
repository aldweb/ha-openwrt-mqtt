#!/bin/sh
# ==============================================================================
# /!\ Script to be used with OpenWrt 25.12, which replaced opkg with apk
# If using OpenWrt 24.10 or earlier, then use previous script version instead
# ==============================================================================

# ============================================================
# CONFIGURATION - Choose publish method:
#   "mqtt"  = native mosquitto_pub (installed via apk)
...
if [ "$PUBLISH_METHOD" = "mqtt" ]; then
    ...
    if ! command -v mosquitto_pub > /dev/null 2>&1; then
        ...
        if apk list --installed | grep -q "libmosquitto-ssl"; then
            echo "libmosquitto-ssl is already installed, installing mosquitto-client-ssl..."
             apk add mosquitto-client-ssl
        elif apk list --installed | grep -q "libmosquitto-nossl"; then
            ...
            apk add libmosquitto-nossl mosquitto-client-nossl
        fi
    else
    ...
    if ! command -v curl > /dev/null 2>&1; then
        ...
        apk update
        apk add curl || { echo "Error: curl installation failed."; exit 1; }
    else
